...
// **=================================================**
let geologicalData = {};
let currentQuestions = []; // Keep for quiz logic later
...
async function loadGeologyData() {
    ...
    // لا نعطل الأزرار هنا بعد، ننتظر لنرى نتيجة التحميل
    try {
        const response = await fetch('./Question.json');
        console.log("التحميل: Fetch response status:", response.status);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        // **تحقق مهم:** تأكد أن نوع المحتوى هو JSON
        const contentType = response.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
            console.error("خطأ: الملف المستلم ليس بتنسيق JSON!", contentType);
            throw new TypeError("Received non-JSON response");
        }

        geologicalData = await response.json(); // محاولة قراءة JSON
        console.log("التحميل: البيانات تم قراءتها كـ JSON بنجاح.");

        if (typeof geologicalData !== 'object' || geologicalData === null || Object.keys(geologicalData).length === 0) {
            console.error("خطأ: بيانات JSON فارغة أو غير صالحة بعد القراءة.");
            throw new Error("JSON data is empty or invalid after parsing.");
        }

        // --- فقط عند النجاح ---
        initializeUIElements(geologicalData); // تهيئة القوائم والنصوص
        setupEventListeners();              // ربط الأزرار
        startCohortCountdown();             // بدء العداد
        setLoadingState(false);             // إخفاء التحميل (وتمكين الأزرار ضمنياً)
        console.log("التحميل: تهيئة الواجهة اكتملت.");

    } catch (error) {
        console.error("فشل تحميل أو معالجة بيانات المهمة:", error);
        // عرض الخطأ للمستخدم
        showNotification(`${translations[currentLanguage].loading_error} (${error.message})`, 6000); // إظهار تفاصيل الخطأ
        // تعطيل الأزرار الرئيسية في حالة الفشل
        setLoadingState(false, true);
    }
}

// دالة التحكم بحالة التحميل (تعطيل/تمكين الأزرار)
function setLoadingState(isLoading, isError = false) {
    ...
    // إخفاء أي رسالة تحميل قديمة في قائمة المواضيع
...
function initializeUIElements(data) {
    ...
    translateUI(currentLanguage); // استدعاء الترجمة لتحديث كل النصوص الثابتة

    // --- تهيئة قائمة المواضيع ---
    const topicsList = document.getElementById('topics-list');
    const sidebarList = document.getElementById('sidebar-topics-list');
    if (!topicsList || !sidebarList) {
        console.error("التهيئة الخطيرة: لم يتم العثور على عنصر قائمة المواضيع.");
        return;
    }
    ...
    Object.keys(data).forEach(topicKey => {
        ...
        if (!Array.isArray(questions)) {
             console.warn(`التهيئة: تخطي '${topicDisplayName}', البيانات ليست مصفوفة.`);
             return;
        }
        console.log(`التهيئة: إضافة الموضوع '${topicDisplayName}'`);

        const startTopicQuizHandler = () => { // دالة مشتركة لبدء الاختبار
            console.log(`الحدث: بدء اختبار الموضوع '${topicDisplayName}'`);
            startQuiz(topicDisplayName, questions);
            closeSidebar(); // إغلاق الشريط الجانبي إذا كان مفتوحاً
        };
        ...
        gridCard.onclick = startTopicQuizHandler;
        ...
        sidebarLink.onclick = (e) => { e.preventDefault(); startTopicQuizHandler(); };
...
function setupEventListeners() {
    ...
    let errors = 0; // عداد للتحقق من الأخطاء
    ...
    if (mainCTA) mainCTA.onclick = handleMainCTA; else { console.error("خطأ الربط: #start-challenge-btn"); errors++; }
    if (secondaryCTA) secondaryCTA.onclick = showMissionBrief; else console.warn("تحذير الربط: #view-brief-btn");
    if (dailyChallengeBtn) dailyChallengeBtn.onclick = startDailyChallenge; else { console.error("خطأ الربط: #daily-challenge-btn"); errors++; }
    if (showTopicsBtn) showTopicsBtn.onclick = showTopicsListScreen; else { console.error("خطأ الربط: #show-topics-btn"); errors++; }
    if (backToHeroBtn) backToHeroBtn.onclick = showHeroScreen; else { console.error("خطأ الربط: #back-to-hero-btn"); errors++; }

    // --- أزرار الاختبار ---
    const submitBtn = document.getElementById('submit-btn');
    const nextBtn = document.getElementById('next-btn');
    if (submitBtn) submitBtn.onclick = handleSubmitAnswer; else { console.error("خطأ الربط: #submit-btn"); errors++; }
    if (nextBtn) nextBtn.onclick = handleNextQuestion; else { console.error("خطأ الربط: #next-btn"); errors++; }

    // --- الشريط الجانبي ---
    const openSidebarBtn = document.getElementById('open-sidebar-btn');
    const closeSidebarBtn = document.getElementById('close-sidebar-btn');
    const overlay = document.getElementById('sidebar-overlay'); // استخدام ID الصحيح
    ...
    // --- أزرار النتائج ---
     ...
     if (!restartBtn) console.warn("تحذير الربط: لم يتم العثور على زر إعادة المحاولة.");
     if(shareBtn) shareBtn.onclick = generateSharableSummary; else console.warn("تحذير الربط: لم يتم العثور على #share-results-btn");

    if (errors > 0) {
        console.error(`الربط: فشل ربط ${errors} من العناصر الأساسية! قد لا تعمل بعض الأزرار.`);
        showNotification("حدث خطأ في تهيئة الواجهة. قد لا تعمل بعض الأزرار.", 5000);
    } else {
        console.log("الربط: تم ربط جميع مستمعي الأحداث بنجاح.");
    }
}
...
// **=================================================**
// ... (نفس الدوال السابقة: handleMainCTA, askForCommitment, showTopicsListScreen, showHeroScreen, startOnboardingMission, showMissionBrief, shuffleArray, startDailyChallenge) ...
...
// **=================================================**
// ... (نفس الدوال السابقة: startQuiz, displayQuestion, handleSubmitAnswer, handleNextQuestion, handleTimeout, showResults, unlockBadge, generateSharableSummary) ...
...
// **=================================================**
// ... (startTimer, translateUI, changeLanguage, toggleTheme, showNotification, showElement, hideElement, openSidebar, closeSidebar) ...
...
function openSidebar() { const sidebar = document.getElementById('sidebar'); const overlay = document.getElementById('sidebar-overlay'); if(sidebar) sidebar.classList.add('open'); if(overlay) overlay.style.display = 'block'; console.log("الواجهة: فتح الشريط الجانبي"); } // Updated to use correct overlay ID if needed
function closeSidebar() { const sidebar = document.getElementById('sidebar'); const overlay = document.getElementById('sidebar-overlay'); if(sidebar) sidebar.classList.remove('open'); if(overlay) overlay.style.display = 'none'; console.log("الواجهة: إغلاق الشريط الجانبي"); } // Updated to use correct overlay ID if needed
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadGeologyData(); // This will call initializeUIElements and setupEventListeners on success
     ...
     // Initial language translation happens inside initializeUIElements after data load
});
