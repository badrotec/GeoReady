...
// قاموس الترجمة (*** تم إضافة ترجمة زر الرجوع ***)
const translations = {
    'ar': {
...
// **=================================================**
// [2] تحميل البيانات والتهيئة الأولية (تم تعديله مسبقاً)
// **=================================================**
...
// **=================================================**
// [2.5] دالة ملء القوائم (تم تعديلها مسبقاً)
// **=================================================**
...
// ------ دالة بدء التحدي اليومي (تم تعديلها مسبقاً) ------
...
// ------ معالجة الإجابة (Submit) (*** معدل لتشغيل الصوت ***) ------
const submitBtn = document.getElementById('submit-btn');
if (submitBtn) {
    submitBtn.addEventListener('click', () => {
        ...
        if (isCorrect) {
            ...
            if (correctSound) {
                ...
                correctSound.play().catch(e => console.error("Error playing correct sound:", e)); // معالجة الأخطاء المحتملة
            }
        } else {
            score += POINTS_WRONG;
...
// ------ عرض النتائج النهائية (*** معدل لتشغيل الصوت ***) ------
...
function startTimer() {
    ...
    const timerDisplayElement = document.getElementById('timer-display'); // *** جديد: الحصول على العنصر الأب للمؤقت ***
    const t = translations[currentLanguage];
...
// ------ دالة الترجمة (تم تعديلها مسبقاً) ------
...
// **=================================================**
// [5] تشغيل الكود عند تحميل الصفحة (*** معدل لتفعيل زر العودة وعداد النشطين ***)
// **=================================================**
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // *** جديد: تفعيل زر العودة من شاشة الاختبار ***
    ...
    if (restartBtn) {
     ...
     // --- Active users count update (*** منطق جديد وتدريجي ***) ---
     const activeUsersCountElement = document.getElementById('active-users-count');
     function updateActiveUsersGradually() {
         // حساب تغيير بسيط (+1, -1, or 0)
         let change = Math.floor(Math.random() * 3) - 1; // رقم بين -1 و 1

         currentActiveUsers += change;

         // التأكد من البقاء ضمن النطاق (3 إلى 16)
     ...
     // جدولة التحديثات بفترات عشوائية قليلاً
     function scheduleNextUserUpdate() {
          const randomInterval = Math.random() * 4000 + 3000; // بين 3 و 7 ثواني
          setTimeout(() => {
               updateActiveUsersGradually();
               scheduleNextUserUpdate(); // جدولة التحديث التالي
          }, randomInterval);
     }
     if (activeUsersCountElement) activeUsersCountElement.textContent = currentActiveUsers; // تحديث فوري بالرقم الابتدائي
     scheduleNextUserUpdate(); // بدء الجدولة
     // --- نهاية تحديث عداد المستخدمين ---
